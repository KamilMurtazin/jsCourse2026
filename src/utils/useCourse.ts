...
import type { LogEntry, RunStatus, TaskOption } from '../types';
...
export function useCourse() {
    ...
    const visibleLogs = computed(() => logs.value.filter(item => logFilter.value === 'all' || item.level === logFilter.value));
    const lines = computed(() => source.value.replace(/\r\n/g, '\n').split('\n'));
    const isNode = computed(() => task.value?.type === 'js' && requiresNode(source.value));
    const testCommand = computed(() => task.value ? `yarn test "${task.value.directory}"` : `yarn test "Lessons/${lesson.value}"`);
    ...
    function addLog(level: LogEntry['level'], text: string) {
        logs.value.push({ id: ++logId, level, text: text.slice(0, 20000), time: new Date().toLocaleTimeString('ru-RU') });
    ...
    function stop(mark = true) {
        ...
        frameUrl.value = '';
        slow.value = false;
        if (mark) status.value = 'stopped';
    }
    function saveUrl(replace = false) {
        const url = new URL(location.href);
        if (lesson.value) url.searchParams.set('lesson', lesson.value); else url.searchParams.delete('lesson');
        if (taskId.value) url.searchParams.set('task', taskId.value); else url.searchParams.delete('task');
        if (url.href !== location.href) history[replace ? 'replaceState' : 'pushState']({}, '', url);
        writeStorage('course:last', { lesson: lesson.value, task: taskId.value });
    }
    async function loadSelection() {
        ...
        if (!lesson.value) { loading.value = false; return; }
        loading.value = true;
        try {
            if (!selected) {
                ...
                if (current === revision) lessonDescription.value = html;
            } else {
                const results = await Promise.allSettled([selected.codeLoader(), selected.readmeLoader?.() ?? Promise.resolve('')]);
                if (current !== revision) return;
                if (results[0].status === 'rejected') throw results[0].reason;
                source.value = results[0].value;
                const raw = results[1].status === 'fulfilled' ? results[1].value : '';
                const html = await renderMarkdown(raw || 'Условие пока не добавлено. Откройте README.md в папке задачи.', selected.directory);
                if (current !== revision) return;
                description.value = html;
                loading.value = false;
                if (results[1].status === 'rejected') flash('Не удалось загрузить условие. Код задачи доступен.');
                if (isNode.value) status.value = 'node';
                else await run();
            }
        } catch (error) {
            if (current !== revision) return;
            loadError.value = `Не удалось загрузить материалы: ${error instanceof Error ? error.message : String(error)}`;
            status.value = 'error';
    ...
    function restore(initial = false) {
        const url = new URL(location.href);
        const saved = readStorage<{ lesson?: string; task?: string }>('course:last', {});
        ...
        if ((requestedLesson && !lesson.value) || (requestedTask && !taskId.value)) flash('Задача из ссылки не найдена. Выберите её из списка.');
        query.value = '';
        group.value = 'all';
        saveUrl(true);
        void loadSelection();
    ...
    async function run(debug = false) {
        const selected = task.value;
        if (!selected || loading.value || isNode.value) return;
        inputError.value = '';
        if (selected.type === 'js') {
            try { parseArguments(input.value); }
        ...
        timer = setTimeout(() => { slow.value = true; }, 10000);
    }
    ...
    function message(event: MessageEvent) {
        if (event.source !== frame.value?.contentWindow || event.origin !== location.origin
            || event.data?.channel !== 'course-runner' || event.data?.runId !== runId.value || !runId.value) return;
        const data = event.data;
        if (data.type === 'ready') frame.value?.contentWindow?.postMessage({
            channel: 'course-runner', type: 'execute', runId: runId.value,
            path: task.value?.path, input: runInput, debug: runDebug,
        }, location.origin);
        if (data.type === 'log' && ['log', 'info', 'warn', 'error', 'result'].includes(data.level) && typeof data.text === 'string') addLog(data.level, data.text);
    ...
    function saveNote(value: string) {
        ...
        if (!writeStorage(noteKey(), value)) flash('Браузер не разрешил сохранить заметку. Она доступна до смены строки.');
    }
    async function copy(text: string) {
        try { await navigator.clipboard.writeText(text); flash('Скопировано'); }
        catch { flash('Буфер обмена недоступен. Выделите и скопируйте текст вручную.'); }
    }
    function keyboard(event: KeyboardEvent) {
        ...
        if (!walkthrough.value || target?.closest('input, textarea, select, [contenteditable]')) return;
        if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
            event.preventDefault(); moveLine(event.key === 'ArrowDown' ? 1 : -1);
        }
    }
    watch(input, value => {
        inputError.value = '';
        if (!ignoreInputSave && task.value && !writeStorage(storageKey(), value)) flash('Не удалось сохранить аргументы в браузере.');
    }, { flush: 'sync' });
    watch([activeLine, filePath], () => {
    ...
    return { lessons, lesson, taskId, tasks, task, query, group, filteredTasks, source, description, lessonDescription,
