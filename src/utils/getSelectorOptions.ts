...
const lessonReadmes = import.meta.glob<string>('../../Lessons/*/README.md', {
    query: '?raw',
    import: 'default',
});
...
const readmes = import.meta.glob<string>('../../Lessons/*/{Tasks,Homework}/*/README.md', {
    query: '?raw',
    import: 'default',
});
export function getLessonsOptions(): LessonOption[] {
    return Object.entries(lessonReadmes)
        .map(([path, loader]) => ({
            id: path.split('/')[3],
            path,
            loader,
...
export function buildTasksMap(
    lessons: LessonOption[],
    files: Record<string, TextLoader>,
    descriptions: Record<string, TextLoader>,
) {
    const map = new Map<string, TaskOption[]>(lessons.map((lesson) => [lesson.id, []]));
    for (const [path, codeLoader] of Object.entries(files)) {
        const match = path.match(
            // eslint-disable-next-line @stylistic/max-len -- Keep the catalog path grammar in one expression.
            /^\.\.\/\.\.\/Lessons\/([^/]+)\/(Tasks|Homework)\/(Task (\d+)\. [^/]+)\/(?:(src|Example)\/)?(index\.(js|ts|html)|App\.vue)$/,
        );
        ...
        map.get(lesson)!.push({
            ...
            lesson,
            group: group as TaskOption['group'],
            num: Number(num),
            extension,
            type: (['html', 'vue'].includes(extension) ? extension : 'js') as TaskOption['type'],
            path: path.slice(6),
            directory,
            codeLoader,
    ...
    for (const tasks of map.values()) {
        tasks.sort(
            (a, b) => a.group.localeCompare(b.group) * -1 || a.num - b.num || a.id.localeCompare(b.id),
        );
    }
    return map;
}
