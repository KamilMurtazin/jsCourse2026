...
const highlightedLines = computed(() => lines.value.map(
    (line) => hljs.highlight(line, {
        language: task.value?.type === 'js' ? 'typescript' : 'xml',
        ignoreIllegals: true,
    }).value || ' ',
));
...
watch(activeLine, async () => {
    ...
    await nextTick();
    document.querySelector(`[data-line="${activeLine.value}"]`)?.scrollIntoView({
        block: 'nearest',
        inline: 'nearest',
    });
});
    ...
    <div class="course" :class="{ 'is-presenting': presentation }">
        ...
        <aside class="sidebar" aria-label="Навигация по курсу">
            <div class="sidebar-title">
                <span class="eyebrow">ПРОГРАММА КУРСА</span><span class="count">{{ lessons.length }}</span>
            </div>
            <label for="lesson-selector">Текущий урок</label>
            <select
                id="lesson-selector"
                :value="lesson"
                @change="chooseLesson(($event.target as HTMLSelectElement).value)"
            >
                <option value="">Выберите урок</option>
                <option v-for="entry in lessons" :key="entry.id" :value="entry.id">{{ entry.id }}</option>
            </select>
            <template v-if="lesson">
                <button class="lesson-overview" :class="{ active: !taskId }" @click="chooseTask('')">
                    ↗ Материалы урока
                </button>
                ...
                <div class="task-list-label">
                    <span>ЗАДАЧИ</span><span>{{ filteredTasks.length }} / {{ tasks.length }}</span>
                </div>
                <nav class="task-list" aria-label="Задачи урока">
                    ...
                    >
                        ...
                        <span class="task-item-info"
                            ><strong>{{ item.title.replace(/^Task \d+\. /, '') }}</strong>
                            <small
                                >{{ item.group === 'Tasks' ? 'Практика' : 'Домашняя работа' }}
                                <span>· {{ item.extension.toUpperCase() }}</span></small
                            >
                        </span>
                    </button>
                </nav>
                <p v-if="!filteredTasks.length" class="muted empty-search">
                    Задач не найдено. Измените название или фильтр.
                </p>
            </template>
        ...
        <main id="workspace" class="workspace">
            <div class="breadcrumb">
                <span>Учебная мастерская</span><span v-if="lesson">/</span
                ><span v-if="lesson">Урок {{ lesson.split('.')[0] }}</span
                ><span v-if="task">/</span
                ><span v-if="task">{{ task.group === 'Tasks' ? 'Практика' : 'Домашняя работа' }}</span>
            </div>
            <div class="workspace-heading">
                <div>
                    <p class="eyebrow">{{ task ? 'ПРАКТИКА В КОДЕ' : 'ОТ ТЕОРИИ К ПРАКТИКЕ' }}</p>
                    <h1>
                        {{
                            task
                                ? task.title.replace(/^Task \d+\. /, '')
                                : lesson || 'Начнём с первого запуска'
                        }}
                    </h1>
                    <p class="subtitle">
                        {{
                            task
                                ? 'Прочитайте условие. Измените код. Посмотрите результат.'
                                : 'Условия, ваш код и результат — в одном месте.'
                        }}
                    </p>
                </div>
                <div v-if="task" class="task-navigation">
                    ...
                    <span
                        >{{ position + 1 }} <span class="muted">/ {{ tasks.length }}</span></span
                    >
                    <button
                        class="icon-button"
                        aria-label="Следующая задача"
                        :disabled="position >= tasks.length - 1"
                        @click="moveTask(1)"
                    >
                        →
                    </button>
            ...
            <div v-else-if="loadError" class="notice error" role="alert">
                {{ loadError }} <button class="button" @click="loadSelection">Повторить загрузку</button>
            </div>
            ...
            <section v-if="!lesson" class="welcome panel">
                ...
                <p>
                    Выберите урок и задачу слева. Приложение само запустит решение и покажет, что получилось.
                </p>
                <div class="welcome-steps">
                    <div>
                        <b>01</b>
                        <h3>Разберитесь</h3>
                        <p>Прочитайте условие и примеры.</p>
                    </div>
                    <div>
                        <b>02</b>
                        <h3>Попробуйте</h3>
                        <p>Напишите решение в редакторе.</p>
                    </div>
                    <div>
                        <b>03</b>
                        <h3>Проверьте</h3>
                        <p>Сравните результат и запустите тесты.</p>
                    </div>
                </div>
                <button class="button primary" @click="chooseLesson(lessons[0]?.id ?? '')">
                    Открыть первый урок →
                </button>
            </section>
            ...
            <section v-else-if="!task" class="panel lesson-material">
                ...
                <p v-if="!loading && !tasks.length" class="notice">
                    В этом уроке пока нет задач для запуска.
                </p>
            </section>
            ...
            <template v-else-if="!loading && !loadError">
                <div class="workspace-tools">
                    <div class="task-kind">
                        <span class="language-dot"></span>{{ task.extension.toUpperCase() }}
                        <span class="muted">· автозапуск при выборе</span>
                    </div>
                ...
                <div class="study-grid">
                    <section class="panel instructions">
                        <div class="panel-heading">
                            <h2><span class="section-number">01</span> Условие</h2>
                        ...
                        <details class="test-help">
                            ...
                            <p class="muted">
                                Завершённый запуск сам по себе не означает, что задача решена верно.
                            </p>
                        </details>
                    </section>
                    ...
                    <div class="execution-column">
                        <section class="panel source-panel">
                            ...
                            <div v-if="walkthrough" class="walkthrough">
                                <div class="walkthrough-toolbar">
                                    <strong
                                        >Строка {{ activeLine }}
                                        <span class="muted">из {{ lineCount }}</span></strong
                                    >
                                    <div>
                                        <button
                                            class="icon-button"
                                ...
                                <label for="line-note"
                                    >Заметка к строке
                                    <span class="muted">· сохраняется в этом браузере</span></label
                                >
                                <textarea
                                    id="line-note"
                                    :value="lineNote"
                                    rows="2"
                                    placeholder="Что происходит на этой строке? Как изменится результат?"
                                    @input="saveNote(($event.target as HTMLTextAreaElement).value)"
                                ></textarea>
                                <p class="muted microcopy">
                                    Это разбор текста кода. Паузы выполнения и переменные доступны в DevTools.
                                </p>
                            </div>
                            <form class="run-controls" @submit.prevent="run()">
                                <template v-if="task.type === 'js' && !isNode">
                                    <label for="task-arguments"
                                        >Аргументы функции
                                        <span class="muted">· JSON через запятую</span></label
                                    >
                                    <input
                                        id="task-arguments"
                                        v-model="input"
                                        placeholder='Например: 42, "текст", [1, 2]'
                                        :aria-invalid="!!inputError"
                                        aria-describedby="arguments-help arguments-error"
                                    />
                                    <p id="arguments-help" class="muted microcopy">
                                        Пустое поле — payload из модуля или вызов без аргументов. Массив [1,
                                        2] — один аргумент.
                                    </p>
                                    <p
                                        v-if="inputError"
                                ...
                                <div v-if="isNode" class="notice">
                                    Эта задача использует Node.js. Проверяйте её в терминале:<code
                                        class="command"
                                        >{{ testCommand }}</code
                                    >
                                </div>
                                <template v-else>
                                    <p v-if="domTask" class="microcopy">
                                        Нужны элементы страницы?
                                        <button
                                            type="button"
                                            class="text-button"
                                            @click="chooseTask(domTask.id)"
                                        >
                                            Открыть HTML задачи ↗
                                        </button>
                                    </p>
                                    ...
                                    <details v-if="task.type === 'js'" class="debug-help">
                                        <summary>Пошаговое выполнение в DevTools</summary>
                                        <p>
                                            Откройте инструменты разработчика (F12), затем нажмите «Отладка».
                                            Браузер остановится перед вызовом функции. Нажимайте F11 («Шаг
                                            внутрь»), чтобы войти в код задачи, F10 — для следующего шага, F8
                                            — чтобы продолжить.
                                        </p>
                                        <p>
                                            Переменные находятся в Scope. Можно поставить точку останова
                                            непосредственно в файле задачи во вкладке Sources. Без открытых
                                            DevTools кнопка просто запускает код.
                                        </p>
                                    </details>
                                </template>
                            </form>
                        ...
                        <section class="panel console-panel">
                            ...
                            <div class="console-status">
                                <span class="status" :class="status" role="status"
                            ...
                            <div v-if="slow" class="notice">
                                Задача выполняется больше 10 секунд. Проверьте Promise, сеть или паузу
                                отладчика. Можно остановить запуск.
                            </div>
                            <div
                            ...
                            >
                                ...
                                >
                                    ...
                                    ><span class="log-level">{{
                                        entry.level === 'result' ? '↳' : entry.level
                                    }}</span>
                                    <pre>{{ entry.text }}</pre>
                                </div>
                                <p v-if="!visibleLogs.length" class="console-empty">
                                    {{
                                        logs.length
                                            ? 'Нет сообщений с выбранным фильтром.'
                                            : 'Здесь появятся console.log, ошибки и результат функции.'
                                    }}
                                </p>
                            </div>
                        </section>
                        ...
                        <section v-if="task.type !== 'js'" class="panel preview-panel">
                            <div class="panel-heading">
                                <h2>Результат на странице</h2>
                                <span class="badge">{{ task.extension.toUpperCase() }}</span>
                            </div>
            ...
            <div v-if="task && frameUrl" :class="task.type === 'js' ? 'script-frame' : 'frame-container'">
                ...
                <iframe
                    ...
                    :key="runId"
                    :src="frameUrl"
                    title="Среда выполнения JavaScript"
                    @load="frameLoaded"
                ></iframe>
            </div>
        </main>
