...
4) Прогноз погоды [Пример прогноза погоды](https://cocobytes.github.io/Weater-App/)
   ...
   (например: `https://api.openweathermap.org/data/2.5/weather?lat=<широта>&lon=<долгота>&appid=<YOUR_API_KEY>`).
   Параметры `lat` и `lon` можно получить от пользователя только после его явного согласия.
   Создайте собственный ключ по документации сервиса. Не публикуйте действующие ключи в
   учебных материалах или истории Git.
